const K="aromatic_pos_complete_v2";const LEGACY_KEYS=["aromatic_pos_complete_v1","aromatic_pos_complete_v1_backup","aromatic_pos_complete_v2_backup"];let db={items:[],bills:[],customers:[],purchases:[],duePayments:[],operators:[{id:"OP001",name:"Admin",pin:"1234",role:"Admin"}],settings:{shop:"AROMATIC POS",address:"",phone:"",gstin:"",upi:"9353689775@upi",prefix:"INV",currency:"₹",gst:5,printer:"58",authorization:"",digitalSignature:""}};let bill=[],pay="Cash",payData={};let op=null;const $=x=>document.getElementById(x),M=n=>`${db.settings.currency}${Number(n||0).toFixed(2)}`,today=()=>new Date().toISOString().slice(0,10);function save(){localStorage.setItem(K,JSON.stringify(db));localStorage.setItem(K+"_backup",JSON.stringify(db))}function normalizeDB(x){if(!x||typeof x!=="object")return null;let d={...db,...x,settings:{...db.settings,...(x.settings||{})}};d.items=(d.items||[]).map(i=>({...i,modelName:i.modelName||"",actualMrp:i.actualMrp??i.mrp??0,deliveryDate:i.deliveryDate||"",guarantee:i.guarantee||"",openingStock:Number(i.openingStock??i.stock??0)}));d.bills=d.bills||[];d.customers=d.customers||[];d.purchases=d.purchases||[];d.duePayments=d.duePayments||[];d.operators=d.operators?.length?d.operators:[{id:"OP001",name:"Admin",pin:"1234",role:"Admin"}];return d}function load(){
  try{
    const candidates=[];
    for(let n=0;n<localStorage.length;n++){
      ...
      try{
        ...
        if(x && typeof x==="object" && (Array.isArray(x.items)||Array.isArray(x.bills)||Array.isArray(x.customers)||Array.isArray(x.purchases)||Array.isArray(x.duePayments))){
          const score=(x.items?.length||0)*1000000+(x.bills?.length||0)*10000+(x.customers?.length||0)*100+(x.purchases?.length||0)*10+(x.duePayments?.length||0);
          candidates.push({key,x,score});
        }
      }catch(_){}
    }
    ...
    if(best){
      db=normalizeDB(best.x)||db;
      if(best.key!==K) save();
      console.log("AROMATIC storage loaded from:",best.key,"items:",db.items.length,"bills:",db.bills.length,"customers:",db.customers.length);
    }
  }catch(e){console.error("Storage load failed",e)}
}function uid(p){return p+Date.now().toString(36)+Math.random().toString(36).slice(2,5)}function esc(s){return String(s??"").replace(/[&<>"']/g,c=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"}[c]))}
window.login=login;
...
function reports(){
  let f=$("reportFrom").value,
      t=$("reportTo").value,
      bs=db.bills.filter(b=>b.date.slice(0,10)>=f&&b.date.slice(0,10)<=t),
      sales=bs.reduce((a,b)=>a+b.grand,0),
      gst=bs.reduce((a,b)=>a+b.gst,0),
      disc=bs.reduce((a,b)=>a+Number(b.itemDiscount||0)+Number(b.od||0),0),
      profit=bs.reduce((s,b)=>s+b.items.reduce((z,x)=>{
        let i=db.items.find(y=>y.id===x.itemId);
        return z+x.qty*(x.rate-(i?.purchase||0))-(x.discount||0)
      },0),0);

  let pm={Cash:0,UPI:0,Card:0,Credit:0,Multiple:0};
  bs.forEach(b=>pm[b.payment]=(pm[b.payment]||0)+b.grand);

  let map={};
  bs.forEach(b=>b.items.forEach(x=>{
    let i=db.items.find(y=>y.id===x.itemId);
    map[i?.name||"Item"]=(map[i?.name||"Item"]||0)+x.qty
  }));
  ...
  $("reportResult").innerHTML=`
    <div class="report-summary-grid">
      <div class="report-stat">
        <span>Sales</span>
        <strong>${M(sales)}</strong>
      </div>
      <div class="report-stat">
        <span>Bills</span>
        <strong>${bs.length}</strong>
      </div>
      <div class="report-stat">
        <span>GST</span>
        <strong>${M(gst)}</strong>
      ...
      <div class="report-stat">
        <span>Profit</span>
        <strong>${M(profit)}</strong>
      </div>
    </div>

    <div class="report-panel">
      <div class="report-panel-title">Payment Summary</div>
      <div class="payment-report-grid">
        <div><span>Cash</span><strong>${M(pm.Cash)}</strong></div>
        <div><span>UPI</span><strong>${M(pm.UPI)}</strong></div>
        <div><span>Card</span><strong>${M(pm.Card)}</strong></div>
        <div><span>Due</span><strong>${M(pm.Credit)}</strong></div>
        <div><span>Multiple</span><strong>${M(pm.Multiple)}</strong></div>
      </div>
      <div class="discount-report-row">
        <span>Total Discount</span>
        <strong>${M(disc)}</strong>
      </div>
    </div>

    <div class="report-panel">
      <div class="report-panel-title">Item-wise / Top Selling</div>
      <div class="report-table-wrap">
        <table class="report-table">
          <thead><tr><th>Item</th><th class="num">Qty</th></tr></thead>
          <tbody>
            ${top.map(x=>`<tr><td>${esc(x[0])}</td><td class="num">${x[1]}</td></tr>`).join("")}
          </tbody>
        </table>
      </div>
    </div>
    ...
    <div class="report-panel">
      <div class="report-panel-title">Stock Report</div>
      <div class="report-table-wrap">
        <table class="report-table">
          <thead>
            <tr><th>Item</th><th class="num">Stock</th><th class="num">Cost Value</th><th class="status-col">Status</th></tr>
          </thead>
          <tbody>
            ${db.items.map(i=>`
              <tr>
                ...
                <td class="num">${M(i.stock*i.purchase)}</td>
                <td class="status-col"><span class="${i.stock<=i.min?"stock-low":"stock-ok"}">${i.stock<=i.min?"LOW":"OK"}</span></td>
              </tr>`).join("")}
          </tbody>
        </table>
      </div>
...
function printReport(){
  let w=window.open("","_blank");
  let shop=db.settings?.shop||"AROMATIC POS";
...
<head>
<meta charset="UTF-8">
<title>${esc(shop)} - Sales Report</title>
<style>
*{box-sizing:border-box}
body{font-family:Arial,Helvetica,sans-serif;margin:0;padding:24px;color:#111827;background:#fff;font-size:12px}
.report-print{max-width:900px;margin:auto}
.report-head{display:flex;justify-content:space-between;align-items:flex-start;border-bottom:2px solid #111827;padding-bottom:12px;margin-bottom:16px}
.report-head h1{font-size:22px;margin:0 0 4px}
.report-head .sub{color:#6b7280;font-size:11px}
.report-head .period{text-align:right;font-size:11px;line-height:1.7}
.report-summary-grid{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:14px}
.report-stat{border:1px solid #d1d5db;border-radius:8px;padding:11px;background:#f9fafb}
.report-stat span{display:block;color:#6b7280;font-size:10px;margin-bottom:5px}
.report-stat strong{font-size:16px}
.report-panel{border:1px solid #d1d5db;border-radius:8px;margin-bottom:14px;overflow:hidden}
.report-panel-title{font-weight:700;font-size:13px;padding:10px 12px;background:#f3f4f6;border-bottom:1px solid #d1d5db}
.payment-report-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:0}
.payment-report-grid>div{padding:10px 12px;border-right:1px solid #e5e7eb}
.payment-report-grid>div:last-child{border-right:0}
.payment-report-grid span{display:block;color:#6b7280;font-size:10px;margin-bottom:4px}
.payment-report-grid strong{font-size:12px}
.discount-report-row{display:flex;justify-content:space-between;border-top:1px solid #e5e7eb;padding:10px 12px;font-weight:600}
.report-table{width:100%;border-collapse:collapse}
.report-table th,.report-table td{padding:8px 10px;border-bottom:1px solid #e5e7eb;text-align:left}
.report-table th{background:#f9fafb;font-size:10px;text-transform:uppercase}
.report-table tr:last-child td{border-bottom:0}
.report-table .num{text-align:right;white-space:nowrap}
.status-col{text-align:center!important}
.stock-ok,.stock-low{font-weight:700}
.report-foot{text-align:center;color:#6b7280;font-size:10px;margin-top:18px}
@media print{body{padding:0}.report-print{max-width:none}.report-panel{break-inside:avoid}.report-stat{background:#fff}}
@media(max-width:650px){.report-summary-grid{grid-template-columns:repeat(2,1fr)}.payment-report-grid{grid-template-columns:repeat(2,1fr)}.payment-report-grid>div{border-right:0;border-bottom:1px solid #e5e7eb}.report-head{display:block}.report-head .period{text-align:left;margin-top:8px}}
</style>
</head>
<body>
<div class="report-print">
  <div class="report-head">
    <div>
      <h1>${esc(shop)}</h1>
      <div class="sub">Sales & Stock Report</div>
    </div>
    <div class="period"><b>Report Period</b><br>${esc(from)} to ${esc(to)}<br>Generated: ${new Date().toLocaleString()}</div>
  </div>
  ${$("reportResult").innerHTML}
  <div class="report-foot">Computer generated report</div>
</div>
</body></html>`);
  w.document.close();
...
function addOperator(){let id=$("opId").value.trim(),name=$("opName").value.trim(),pin=$("opPin").value.trim();if(!id||!name||!pin)return alert("Fill operator fields");db.operators.push({id,name,pin,role:$("opRole").value});save();renderSettings()}function importLoginBackup(e){let file=e.target.files?.[0];if(!file)return;let r=new FileReader();r.onload=()=>{try{let x=JSON.parse(r.result);if(!x.items||!x.bills)throw 0;if(confirm("Import this backup into this browser? Current local data will be replaced.")){db=normalizeDB(x)||db;save();alert("Backup imported successfully. Please login again.");location.reload()}}catch(_){alert("Invalid backup file")}};r.readAsText(file);e.target.value=""}function exportBackup(){let a=document.createElement("a");a.href=URL.createObjectURL(new Blob([JSON.stringify(db,null,2)],{type:"application/json"}));a.download=`AROMATIC_POS_Backup_${today()}.json`;a.click()}function importBackup(e){let r=new FileReader();r.onload=()=>{try{let x=JSON.parse(r.result);if(!x.items||!x.bills)throw 0;if(confirm("Restore this backup?")){db=x;save();refresh();alert("Restored")}}catch(_){alert("Invalid backup")}};r.readAsText(e.target.files[0])}function wipe(){if(op?.role!=="Admin")return alert("Admin only");if(confirm("Delete ALL data?")){localStorage.removeItem(K);sessionStorage.clear();location.reload()}}function closeModal(){$("modal").classList.add("hidden")}
...
window.__AROMATIC_SCRIPT_LOADED__=true;
